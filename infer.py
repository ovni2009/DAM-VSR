...
from src.pipelines.pipeline_vsr_tile import FrameInterpolationWithNoiseInjectionPipeline
from src.schedulers.scheduling_euler_discrete import EulerDiscreteScheduler
from src.attn_ctrl.attention_control import (AttentionStore, 
...
def unsharp_mask(tensor, sigma=1.0, strength=1.5):
    ...
    sharpened_tensor = torch.tensor(np.array(sharpened_frames)).permute(0, 3, 1, 2)
    ...
    for video_path in validation_video_paths:
        ...
        for i in range(0, video_tensor.shape[0], args.frames_chunk - args.overlap):
            ...
            if generated is not None and i + args.frames_chunk > video_tensor.shape[0]:
                # cur_i = max(video_tensor.shape[0] + args.overlap - args.frames_chunk -1 , 0) # TODO
                cur_i = max(video_tensor.shape[0] - args.frames_chunk, 0)
            ...
            if star_index not in HQ_Refer_dict:
                ...
                ref_image = HQ_Refer_dict[star_index]
            
            if end_index not in HQ_Refer_dict:
                if args.sr_type=='supir':
                    end_image = get_hqReference(args, transforms.ToPILImage()(torch.nn.functional.interpolate(video_tensor[end_index].unsqueeze(0), size=[video_tensor.shape[2]*args.video_upscale, video_tensor.shape[3]*args.video_upscale], mode="bicubic")[0].clamp(0,1)).convert('RGB'), sr_model, llava_agent, use_llava, args.device)
                elif args.sr_type=='resshift':
                    input_img = video_tensor[end_index].unsqueeze(0).to(device=args.device)
                    end_image = resshift_sampler.inference_prepare(
                        input_img
                        )
                elif args.sr_type=='invsr':
                    input_img = video_tensor[end_index].unsqueeze(0).to(device=args.device)
                    end_image = invsr_sampler.inference_prepare(
                        input_img
                        )
                    end_image = torch.tensor(end_image).permute(0, 3, 1, 2)

                HQ_Refer_dict[end_index] = end_image.cpu()
            else:
                end_image = HQ_Refer_dict[end_index]

            run_time = time.time() - start_time
            print(f'time (sec): {run_time:.2f} \n')

            generated = pipe(unet=unet, image1=ref_image, image2=end_image, 
            ...
            if i != 0:
                ...
                input_depth = input_depth[cur_overlap:]
                print(f"input_depth.shape={input_depth.shape}")
            
            results.append(generated.cpu())
            cond_results.append(input_depth.cpu())


        
        frames_output = torch.cat(results, dim=0).cpu()
        frames_depth = torch.cat(cond_results, dim=0).cpu()
        ...
        frames_depth = frames_depth[:, :, pad_h1*args.video_upscale:frames_depth.shape[2]-pad_h2*args.video_upscale, pad_w1*args.video_upscale:frames_depth.shape[3]-pad_w2*args.video_upscale]


        if args.color_fix_type == 'Wavelet':
            res_processing = wavelet_reconstruction(frames_output.float(), frames_depth.float()).clamp(0, 1)
        elif args.color_fix_type == 'AdaIn':
            res_processing = adaptive_instance_normalization(frames_output.float(), frames_depth.float()).clamp(0, 1)
        else:
